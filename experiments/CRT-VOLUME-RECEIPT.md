...
- One camera distance derived from `FRAME_H = 7.8`; nothing dollies. The aim slides to the left bezel rail as the frame narrows (`aimX = Math.min(0, LEFT_EDGE + frameW / 2)`) with `lookAt` re-issued every frame.
